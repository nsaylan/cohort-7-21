...
```bash
docker build -t "nsaylan/flask-app:1.0" .
docker image ls
```
...
```bash
docker run -d --name welcome -p 80:80 nsaylan/flask-app:1.0
docker container ls
```
...
```bash
docker push nsaylan/flask-app:1.0
```
...
```bash
docker build -t "nsaylan/flask-app:2.0" -f ./Dockerfile-alpine . #-f tag file name demek. dockfile dışında bir belge kullanılacaksa kullanılır.
docker image ls
```

- Note that while the size of `nsaylan/flask-app:1.0` is approximately 400MB, the size of `nsaylan/flask-app:2.0` is 56MB.

- Run the newly built image as container in detached mode, connect host `port 80` to container `port 80`, and name container as `welcome`. Then list running containers and connect to EC2 instance from the browser to show the Flask app is running.

```bash
docker run -d --name welcome2 -p 8080:80 nsaylan/flask-app:2.0
docker ps
```
...
```bash
docker push nsaylan/flask-app:2.0
```
